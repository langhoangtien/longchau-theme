...
export default async function Footer() {
  ...
  const dataMenuMapped = data.menu?.map((item: any) => ({
    name: item.name,
    link: `danh-muc/${item.code}-${item._id}`,
  }));
  const dataAbout = data.pages?.map((item: any) => ({
    name: item.name,
    link: item.slug,
  }));

  const FOOTER_MENU = [{ title: "Về chúng tôi", items: dataAbout }].concat(
    listMenu,
    { title: "Danh mục", items: dataMenuMapped }
  );
  return (
    <footer className="w-full ">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        {/*Grid*/}
        <div className="w-full border-t mt-8 border-gray-300"></div>
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 gap-y-8 md:gap-8 py-10 max-w-sm mx-auto sm:max-w-3xl lg:max-w-full">
          <div className="col-span-full mb-10 lg:col-span-2 flex flex-col space-y-4 lg:mb-0">
            <Link href="/" className="flex justify-center lg:justify-start">
              <Logo width={120} height={60} />
            </Link>
            <p className="text-sm text-gray-500 lg:max-w-xs text-center lg:text-left">
              Được tin tưởng bởi 30 doanh nghiệp &amp; 120 nghìn khách hàng. Bạn
          ...
          {FOOTER_MENU.map((menu) => (
            <div key={menu.title} className="lg:mx-auto text-left ">
              <h4 className="text-lg uppercase text-gray-900 font-medium mb-7">
                {menu.title}
              </h4>
          ...
          <div className="lg:mx-auto text-left ">
            ...
            {FOOTER_SWITCHBOARD_MENU.map((item) => (
              <div
                key={item.name}
                className="text-sm text-gray-500 leading-6 mb-2"
              >
                <p className="inline-block py-[2px] md:text-sm ">{item.name}</p>
                <a className="inline-block" href={`tel:${item.phone}`}>
        ...
        <div className="py-7 border-t border-gray-200">
          <div className="flex items-center justify-center flex-col lg:justify-between lg:flex-row">
            <span className="text-sm text-gray-500 ">
              ©<Link href="/">Ludmila</Link> 2024, All rights reserved.
            </span>
